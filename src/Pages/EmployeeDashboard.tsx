...
import TaxDeclaration from "./TaxDeclarationPage";
import TaxResult from "./TaxResult";
import { userData } from "../Data/UserData";
import { useState } from "react";

const EmployeeDashboard: React.FC<{ user: userData }> = ({ user }) => {
  const [userId, setUserId] = useState<string>(user.userId);
  return (
    <div className="container-fluid mx-0 px-0 justify-content-center">
      <header className="d-flex flex-wrap align-items-center justify-content-between justify-content-md-between py-3 px-4 border-bottom">
        ...
        <div className="dropdown text-end">
          ...
          <ul className="dropdown-menu text-small">
            <li><Link to={"/taxDeclaration"} className="dropdown-item">Investment Declaration</Link></li>
      ...
      <div>
        <Routes>
          <Route path="/taxDeclaration" element={<TaxDeclaration user={user} />}>
            <Route path="/taxDeclaration/oldRegime" element={<InvestmentDeclaration user={user}/>} />
            <Route path="/taxDeclaration/newRegime" element={<TaxResult user={userId} regimeType="new" />} />
          </Route>
